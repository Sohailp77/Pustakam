...
.start_button, .clear_button, .check_button {
    border: 0;
            ...
            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
                ...
                foreach($passage_arr as $text)
                {
                    ...
                    <div class="row" style="border-bottom:1px solid #000;margin-bottom: 5px; padding: 5px;">
                        <div class="col-xl-10 col-lg-10 col-md-10 col-sm-12" style="align-self: center;">
                        ...
                        <div class="col-xl-2 col-lg-2 col-md-2 col-sm-12" style="align-self: center;">
                          <div class="row">
                            <div class="col-xl-4 col-lg-4 col-md-4 col-sm-4" style="align-self: center;padding-left: 0px;padding-right: 0px;">
                            <button  id="start_button<?=$i?>" class="start_button" onclick="startButton<?=$i?>(event)" title="Record Audio">
                                <img id="start_img<?=$i?>" src="<?=base_url()?>/assets/mic.gif" alt="Start">
                            </button>
                            </div>
                            <div class="col-xl-4 col-lg-4 col-md-4 col-sm-4" style="align-self: center;padding-left: 0px;padding-right: 0px;">
                            <button  class="check_button" onclick="checkButton<?=$i?>(event)" title="Check Audio">
                                <img id="check_img<?=$i?>" src="<?=base_url()?>/assets/check.png" style="max-width:50%;">
                            </button>
                            </div>
                            <div class="col-xl-4 col-lg-4 col-md-4 col-sm-4" style="align-self: center;padding-left: 0px;padding-right: 0px;">
                            <button  class="clear_button" onclick="clearButton<?=$i?>(event)" title="Clear Text" >
                                <img id="clear_img<?=$i?>" src="<?=base_url()?>/assets/clear.png" style="max-width:50%;">
                            </button>
                            </div>
                          </div>
                        </div>
                    ...
                    <script type="text/javascript">
                        ...
                        if (!('webkitSpeechRecognition' in window)) {
                          ...
                          recognition<?=$i?>.onresult = function(event) {
                            ...
                            $('#final_span<?=$i?>').html(linebreak<?=$i?>(final_transcript<?=$i?>));
                            $('#interim_span<?=$i?>').html(linebreak<?=$i?>(interim_transcript<?=$i?>));
                          };
                        }
                        ...
                        function startButton<?=$i?>(event) {
                          $("#final_span<?=$i?>").empty();
                          $("#interim_span<?=$i?>").empty();
                          $("#result<?=$i?>").empty();
                          $("#result<?=$i?>").html("Result Appeared Here....");
                          if (recognizing<?=$i?>) {
                            recognition<?=$i?>.stop();
                            return;
                          }
                        ...
                        function clearButton<?=$i?>(event)
                        {
                          $("#interim_span<?=$i?>").empty();
                          $("#final_span<?=$i?>").empty();
                          $("#final_span<?=$i?>").html("Recorded Text displayed here....");
                          $("#result<?=$i?>").empty();
                          $("#result<?=$i?>").html("Result Appeared Here....");
                        }

                        function checkButton<?=$i?>(event)
                        {
                          checkDiff<?=$i?>();
                        }
